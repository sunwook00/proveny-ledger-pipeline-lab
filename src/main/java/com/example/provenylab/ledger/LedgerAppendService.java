...
public class LedgerAppendService {
  ...
  public AppendResult append(LedgerEvent event) {
    ...
    if (existing != null && existing > 0)
      return AppendResult.duplicateIgnored(event.idempotencyKey());
    jdbc.update(
        "INSERT INTO ledger_chain(passport_id,last_seq,last_hash) VALUES (?,0,NULL) ON CONFLICT (passport_id) DO NOTHING",
        event.passportId());
    Map<String, Object> chain =
        jdbc.queryForMap(
            "SELECT last_seq,last_hash FROM ledger_chain WHERE passport_id=? FOR UPDATE",
            event.passportId());
    ...
    jdbc.update(
        "UPDATE ledger_chain SET last_seq=?, last_hash=?, updated_at=now() WHERE passport_id=?",
        nextSeq,
        entryHash,
        event.passportId());
  ...
  public LedgerEvent fromOutboxPayload(String message) {
    try {
      ...
      return new LedgerEvent(
          (String) root.get("passportId"),
          (String) root.getOrDefault("eventCategory", "OWNERSHIP"),
          (String) root.get("eventAction"),
          (String) root.getOrDefault("actorRole", "SYSTEM"),
          (String) root.getOrDefault("actorId", "system"),
          Instant.parse((String) root.get("occurredAt")),
          payload,
          (String) root.get("idempotencyKey"),
          ((Number) root.getOrDefault("schemaVersion", 1)).intValue());
    } catch (JsonProcessingException e) {
      throw new IllegalArgumentException("Invalid ledger event message", e);
    }
  }
  ...
  private String toJson(Object payload) {
    try {
      ...
      throw new IllegalArgumentException("payload json serialization failed", e);
    }
  }
}
